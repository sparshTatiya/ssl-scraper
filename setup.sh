#!/bin/bash
# ============================================================
# SSL Scraper Setup Script
# Run this from inside your ssl-scraper folder:
#   cd ~/Desktop/ssl-scraper
#   bash setup.sh
# ============================================================

echo "🚀 Setting up SSL Scraper..."
...
name: Scrape SSL Opportunities

on:
  schedule:
    - cron: '0 15 * * *'   # 10:00 AM ET
    - cron: '0 21 * * *'   # 4:00 PM ET
  workflow_dispatch:
  push:
    branches: [main]
  ...
  scrape:
    ...
    steps:
      ...
      - name: Commit and push results
        run: |
          git config user.name "SSL Scraper Bot"
          git config user.email "bot@ssl-finder.local"
          git add docs/
          git diff --cached --quiet || git commit -m "Update opportunities - $(date -u '+%Y-%m-%d %H:%M UTC')"
          git push
EOF
echo "✅ Created .github/workflows/scrape.yml"
...
echo "   1. git add ."
echo "   2. git commit -m 'Add scraper'"
echo "   3. git push"
echo ""
